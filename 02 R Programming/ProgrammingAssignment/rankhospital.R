rankhospital <- function(state, outcome, num = "best") {
        ...
        if (num == "best") num<-1
        if (num == "worst") num<-nrow(state_out_avail)
        ##if (num > nrow(state_out_avail)) stop(NA)
        ...
        state_out_avail[order(as.double(state_out_avail[,3]), state_out_avail$Hospital.Name),][num,1]
